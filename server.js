...
// ========= DataForSEO config =========
const ENABLE_DATAFORSEO = /^true$/i.test(
  process.env.ENABLE_DATAFORSEO || "true"
);
...
const MAX_KEYWORDS_EXPANDED = Math.min(
  200,
  Math.max(1, Number(process.env.MAX_KEYWORDS_EXPANDED || 50))
);
const DFS_SV_BATCH_SIZE = Math.min(
  1000,
  Math.max(1, Number(process.env.DFS_SV_BATCH_SIZE || 200))
);
...
const B_FRESH = Number(process.env.URGENCY_BOOST_FRESH || 0.1);
const B_PAT = Number(process.env.URGENCY_BOOST_PATTERN || 0.1);
...
const DEFAULT_SIGNALS_SOURCES = [
  ...
  {
    url: "https://www.pcisecuritystandards.org/pci_security/rss",
    vertical: "compliance",
    region: "LATAM",
    type: "regulatory",
    ttl_days: 365,
  ...
  {
    url: "https://shopify.engineering/atom.xml",
    vertical: "ecommerce",
    region: "LATAM",
    type: "platform",
    ttl_days: 365,
...
async function bootstrapSources() {
  ...
  if (process.env.SIGNALS_SOURCES_JSON) {
    try {
      ...
      const target =
        process.env.SIGNALS_SOURCES_PATH || "/app/data/signals_sources.json";
      if (await tryWriteJson(target, parsed)) {
        SOURCES_PATH_RESOLVED = target;
        SOURCES_ORIGIN = "env_json";
        return target;
      }
    } catch (e) {
  ...
  if (process.env.SIGNALS_SOURCES_B64) {
    try {
      const parsed = JSON.parse(
        Buffer.from(process.env.SIGNALS_SOURCES_B64, "base64").toString("utf8")
      );
      const target =
        process.env.SIGNALS_SOURCES_PATH || "/app/data/signals_sources.json";
      if (await tryWriteJson(target, parsed)) {
        SOURCES_PATH_RESOLVED = target;
        SOURCES_ORIGIN = "env_b64";
        return target;
      }
    } catch (e) {
  ...
  for (const p of candidatePaths()) {
    try {
      if (fscore.existsSync(p)) {
        SOURCES_PATH_RESOLVED = p;
        SOURCES_ORIGIN = "file_existing";
        return p;
      }
    } catch {}
  }
  ...
  if (await tryWriteJson(fallback, DEFAULT_SIGNALS_SOURCES)) {
    SOURCES_PATH_RESOLVED = fallback;
    SOURCES_ORIGIN = "default_bootstrap";
    return fallback;
  }
  return null;
}
...
app.get("/signals/sources/debug", async (_req, res) => {
  ...
  if (SOURCES_PATH_RESOLVED) {
    try {
      head = (
        await fs.readFile(SOURCES_PATH_RESOLVED, "utf8")
      ).slice(0, 400);
    } catch {}
  }
  res.json({
...
async function dfsSerpFeatures(keyword, pages = 1) {
  ...
  try {
    ...
    const now = Date.now(),
      THRESH = 45;
    const hasNews = items.some((i) =>
      ["top_stories", "news", "google_news"].includes(
    ...
    const freshCount = items.filter((i) => {
      ...
      if (!t) return false;
      const ts = Number(new Date(t));
      return Number.isFinite(ts) && now - ts <= THRESH * 86400000;
    }).length;
    ...
    return {
      ...
      serp_signals: {
        hasNews,
        freshShare: Number(freshShare.toFixed(2)),
        paa,
        related,
      },
    };
...
async function dfsKeywordsForKeywords(seed) {
  if (!ENABLE_DATAFORSEO || !ENABLE_DFS_KFK)
  ...
  try {
    ...
    const items = data?.tasks?.[0]?.result?.[0]?.items || [];
    const kws = items
      .map((it) => String(it.keyword || "").trim())
      .filter(Boolean);
    return {
      keywords: uniqNorm(kws).slice(0, MAX_KEYWORDS_EXPANDED),
      cost: UNIT.DFS_KFK_TASK,
    };
  } catch (e) {
...
function heuristicExpand(topic) {
  ...
  if (t.includes("pci"))
    arr.push(
      ...
      "cumplimiento pci 2025",
      "requisitos pci dss",
      "qsa pci",
      "saq pci dss",
      "controles pci dss",
      "normativa pci pagos",
      "pci dss checklist 2025",
      "procesadores pagos pci",
      "servicio consultoría pci"
    );
  else if (t.includes("shopify"))
    arr.push(
      "shopify checkout extensibility",
      "migración checkout shopify",
      "plantillas checkout shopify",
      "apps checkout shopify",
    ...
    arr.push(
      ...
      "plantillas whatsapp 2025",
      "verificación whatsapp",
      "enrutamiento conversaciones whatsapp",
      "wa cloud api pricing"
    );
  return arr;
}
...
async function dfsSearchVolumeChunks(keywords) {
  if (!ENABLE_DATAFORSEO)
    return {
      ...
      avg_cpc: 1.1,
      trend: 0.12,
      transactional_share: 0.6,
    };

  const { cleaned, rejected } = sanitizeKeywords(keywords);
  if (rejected.length)
    logger.warn({
      msg: "SV batch: rejected invalid keywords",
      count: rejected.length,
      examples: rejected.slice(0, 5),
    });
  const safeKeywords = cleaned.length ? cleaned : [];
  if (!safeKeywords.length)
    return { cost: 0, total_sv: 0, avg_cpc: 0, trend: 0, transactional_share: 0 };

  const auth = { username: DFS_LOGIN, password: DFS_PASSWORD };
  const chunks = [];
  for (let i = 0; i < safeKeywords.length; i += DFS_SV_BATCH_SIZE) {
    chunks.push(safeKeywords.slice(i, i + DFS_SV_BATCH_SIZE));
  }
  ...
  const isTransactional = (kw, cpc, comp) => {
    ...
    return (
      patterns.some((p) => k.includes(p)) || cpc >= 0.5 || comp >= 0.5
    );
  };
    ...
    try {
      ...
      for (const it of items) {
        ...
        for (const m of ms) {
          ...
          const mon =
            m.month || (m.month_num && String(m.month_num).padStart(2, "0"));
  ...
  const transactional_share =
    safeKeywords.length ? transCount / Math.max(1, safeKeywords.length) : 0;
...
function computeDemandIndex(batch) {
  ...
  const demandIdx =
    0.5 * svFactor + 0.2 * trans + 0.2 * trendFactor + 0.1 * cpcFactor;
...
async function getSignals() {
  ...
  try {
    cachedSignals = JSON.parse(
      await fs.readFile(SIGNALS_PATH, "utf8").catch(() => "[]")
    );
  } catch {
    cachedSignals = [];
  }
...
app.post("/score/run", async (req, res) => {
  ...
  if (!parsed.success)
    return res
      .status(400)
      .json({ error: "invalid_input", details: parsed.error.issues });
  ...
  try {
    ...
    const kwSet = uniqNorm([
      topic,
      ...fromSerp,
      ...(kfk.keywords || []),
      ...heur,
    ]).slice(0, MAX_KEYWORDS_EXPANDED);
    ...
    res.json({
      ...
      cost: {
        run_usd: Number(
          ((serp.cost || 0) + (svAgg.cost || 0) + (kfk.cost || 0)).toFixed(3)
        ),
...
app.post("/signals/sources/put", async (req, res) => {
  try {
    ...
    if (!sources)
      return res
        .status(400)
        .json({ ok: false, error: "Provide array or {sources:[...]}" });
    const target =
      process.env.SIGNALS_SOURCES_PATH || "/app/data/signals_sources.json";
    await ensureDir(path.dirname(target));
    await fs.writeFile(target, JSON.stringify(sources, null, 2), "utf8");
    SOURCES_PATH_RESOLVED = target;
    SOURCES_ORIGIN = "put_endpoint";
...
function severityFromText(title) {
  const t = (title || "").toLowerCase();
  if (t.includes("mandatory") || t.includes("obligatorio") || t.includes("deadline"))
    return 5;
  ...
  try {
    ...
    if (!srcPath || !fscore.existsSync(srcPath))
      return res.json({
        ...
        added: 0,
        total: 0,
        note: "signals_sources.json not found (post-bootstrap)",
      });
    ...
    try {
      ...
      return res
        .status(400)
        .json({ ok: false, error: "invalid_sources_json", detail: e.message });
    }
    ...
    for (const s of sources) {
      ...
      try {
        ...
        for (const it of items) {
          ...
          const deadline = yearMatch
            ? `${yearMatch[1]}-12-31`
            : new Date(Date.now() + 90 * 86400000).toISOString().slice(0, 10);
          const id = `auto_${Buffer.from(title + link)
            .toString("base64")
            .slice(0, 12)}`;
...
app.get("/leads/export", async (_req, res) => {
  try {
    ...
    const csv = ["name,email,company,phone,notes,ts"]
      .concat(
        list.map((l) =>
          [
            l.name || "",
...
app.post("/capture/bootstrap", async (req, res) => {
  const { slug, title, subtitle, whatsappIntl, calendlyUrl, ogImage } =
    req.body;
  ...
  await ensureDir(expRoot);
  await ensureDir(expDir);
  try {
    const html = `<!doctype html><html lang="es"><head><meta charset="utf-8"><title>${
      title || "Solución en 21 días"
    }</title>
...
<h1 class="text-4xl font-bold mb-3">${title || ""}</h1><p class="text-slate-600 mb-6">${
      subtitle || ""
    }</p>
<a href="https://wa.me/${
      whatsappIntl || ""
    }" class="inline-block bg-green-500 text-white px-5 py-3 rounded-lg">WhatsApp</a>
</section>
<section class="max-w-3xl mx-auto px-4 pb-12"><iframe src="${
      calendlyUrl || ""
    }" width="100%" height="620"></iframe></section>
...
if(!r.ok) throw new Error(); ok.classList.remove('hidden'); er.classList.add('hidden'); f.reset();
    ...
    await fs.writeFile(
      path.join(expDir, "email_sequence.txt"),
      "Secuencia emails (plantilla)",
      "utf8"
    );
    await fs.writeFile(
      path.join(expDir, "linkedin_sequence.txt"),
      "Secuencia LinkedIn (plantilla)",
      "utf8"
    );
    await fs.writeFile(
      path.join(expDir, "google_ads_editor.csv"),
      "Campaign,...,etc",
      "utf8"
    );
...
// ========= Start =========
app.listen(PORT, () =>
  logger.info(`Market Intel Suite running on :${PORT}`)
);
